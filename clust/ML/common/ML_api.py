...
def test_data_preparation(params, influxdb_client):
    ...
    # 2. Scaling
    test_X, scaler_X , test_y, scaler_y = ML_pipeline.Xy_data_scaling_test(data_X, data_y,
                                                                           scaler_info['scaler_file_path']['XScalerFile']['filePath'],
                                                                           scaler_info['scaler_file_path']['yScalerFile']['filePath'],
                                                                           scaler_info['scaler_flag'])
...
# def test_data_preparation(params, model_meta, influxdb_client):
...
#     """prepare pd.DataFrame data for inference,
#         1. Scale

#     Args:
#         params (dict): it must include 'scaler_info' key.
#         data (ndarray): data to feed into model.

#     Returns:
#         scaled_infer_X (ndarray): sclaed X data
#         scaler_y (): scaler for y data
#     """
#     scaler_info = params['scaler_info']

#     # 1. Scaling
#     scaled_infer_X, scaler_X = _get_scaled_infer_data(data, scaler_info['scaler_file_path']['XScalerFile']["filePath"], scaler_info['scaler_flag'])
#     scaler_y = ml_scaler.get_scaler_file(scaler_info['scaler_file_path']['yScalerFile']["filePath"])
    
#     return scaled_infer_X, scaler_y

def infer_data_preparation(params, data):
    """prepare numpy array data for inference,
        1. Scale

    Args:
        params (dict): parameters including 'scaler_info'.
        data (ndarray): data to feed into model.

    Returns:
        scaled_infer_X (ndarray): sclaed X data
        scaler_y (scaler): scaler for y data
    """
    scaler_info = params['scaler_info']
...
def ml_inference(params, infer_X_array, scaler):
    """inference using given data and model information.
    ...
    if model_info['model_purpose'] == 'regression':
        ...
        preds = ML_pipeline.clust_classification_inference(infer_X_array, model_info)
    target = params['ingestion_param_y']['feature_list']

    if scaler_info['scaler_flag'] =='scale':
