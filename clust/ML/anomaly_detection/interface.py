...
class BaseAnomalyDetModel(abc.ABC):
    """
    Base model class to build a CLUST model for anomaly detection tasks.
    CLUST 플랫폼의 anomlay detection 용 모델 클래스(e.g., RNNPredClust ...) 생성시 상속받아야 할 base class 입니다.
    ...
    # @abc.abstractmethod
    # def anomaly_detect(self) -> Any:
    #     """
    #     이상치 탐지를 위한 ``anomaly_detect`` 메서드 입니다.
    ...
    def create_inferenceloader(self) -> Any:
        """
        추론을 위한 detect data loader 정의를 위한 ``create_detectloader`` 메서드 입니다.
