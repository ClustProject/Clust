...
class FCClust(BaseRegressionModel):
    """
    FC Regression & forecast model class
    """
    def __init__(self, model_params):
        """
        Init function of FC regression class.

        Args:
            model_params (dict): parameters for building an FC model
        """
        self.model_params = model_params
        # model 생성
        self.model = FC(
            representation_size=self.model_params['input_size'],
    ...
    def test(self, test_params, test_loader):
        ...
        with torch.no_grad():
            ...
            for x_test, y_test in test_loader:
                x_test = x_test.view([batch_size, n_features]).to(device)
    ...
    def inference(self, infer_params, inference_loader):
        ...
        with torch.no_grad():
            ...
            for x_infer in inference_loader:
                x_infer = x_infer.view([batch_size, n_features]).to(device)
