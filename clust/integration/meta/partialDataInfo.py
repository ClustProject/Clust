...
class PartialData():
    def __init__(self, partial_data_set, integration_duration_criteria):
        ...
        self.column_meta={}
        self.column_meta['overlap_duration'] = self._get_partial_data_set_start_end(integration_duration_criteria)
    ...
    def _get_partial_data_set_start_end(self, integration_duration_criteria):
        start_list=[]
        end_list =[]
        duration={}
        data_length = len(self.partial_data_set)
        for i in range(data_length):
            ...
            end_list.append(end)
        if integration_duration_criteria == 'common':
            duration['start_time'] = max(start_list)
            duration['end_time'] = min(end_list)
        elif integration_duration_criteria == 'total':
